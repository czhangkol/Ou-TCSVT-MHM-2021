																	...
																	for(int i16=i15;i16<=d+6;i16++)
																	{
																		...
																		for(int j =0;j<=15;j++)
																		{
																			if(j<= m[1] -1)
																			{
																				...
																				if(temp[j] >= bk+2 && temp[j] != d+6)//不连续分布
																				{
																				...
																				else if(temp[j] == bk+1 && temp[j] != d+6)//连续分布
																				{
																					int a = -temp[j]-1,b=temp[j];
																					EC = EC + his[511*j+256+b-1]+his[511*j+256+a-1];
																					ED = ED + (his[511*j+256+b-1]+his[511*j+256+a-1])*2.5 + (cumHis[511*j+256+a-2]+cumHis[511*j+510] -cumHis[511*j+256+b-1])*4;
																				}
																				if(temp[j] == d+6)
																				{
